...
exec > >(tee /work/scratch/dmonopoli/joblogs/${SLURM_JOB_ID}_p3sh.out) 2>&1

source venv_gb10/bin/activate
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True PYTHONUNBUFFERED=1

echo "=== P3 SCALEHEAD start $(date) ==="
python3 -m scripts.train_hand_head --config configs/exp_p3_scalehead.yaml
echo "=== P3 SCALEHEAD done $(date) ==="
